...
const LoadingOverlay = ({ isLoading, onComplete, taskId }) => {
  ...
  // Tips to show during loading with types for styling
  const tips = useMemo(() => [
    {
      text: 'Pro tip: Check the weather forecast before packing!',
      type: 'info'
    },
    {
      text: 'Did you know? Weekday travel is often cheaper than weekends.',
      type: 'success'
    },
    {
      text: 'Travel tip: Always carry a portable charger for your devices.',
      type: 'warning'
    },
    {
      text: '⚠️ Important: Check travel advisories and local COVID-19 guidelines before your trip.',
      type: 'danger'
    },
    {
      text: 'Local tip: Ask locals for their favorite restaurants to find hidden gems.',
      type: 'info'
    },
    {
      text: 'Packing tip: Roll your clothes to save space in your luggage.',
      type: 'info'
    },
    {
      text: '💰 Budget tip: Many museums offer free entry on certain days of the week.',
      type: 'success'
    },
    {
      text: '🚨 Safety tip: Keep digital copies of important documents in case of loss or theft.',
      type: 'danger'
    }
  ], []);

  // Get a random tip with its type
    ...
    <div className={`loading-overlay ${!isLoading ? 'fade-out' : ''}`}>
      <div className="loading-content">
        <div className="loading-header">
          ...
          {currentTip && (
            <p className={`loading-tip ${currentTip.type ? currentTip.type : ''}`}>
              {currentTip.text}
            </p>
          )}
        </div>
